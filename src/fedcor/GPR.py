...
class GPR(torch.nn.Module):
    ...
    def Update_Training_Data(self,client_idxs,loss_changes,epoch):
        ...
        data = np.concatenate([np.expand_dims(np.array(client_idxs),2),
                               np.expand_dims(np.array(loss_changes),2)],2)
...
class Kernel_GPR(GPR):
    ...
    def __init__(self, num_users, loss_type='LOO', init_noise=0.01, reusable_history_length=10, gamma=1.0,
                 device=torch.device('cpu'), dimension=10, kernel=SE_Kernel, **Kernel_Arg):
